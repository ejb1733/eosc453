...
from utilities.geofact import geo_fact
...
def ODEs_T(t,T, VOLC=True, ALBEDO_TEMP_DEPENDENT=True):
    ...
    phi = 1
    if VOLC:
        phi = phi_volcano(t,year=1.75)

    # print(f'GAMMAS: {GAMMAS}, ALBEDOS AVGS: {albedo_avgs}')

    ODEs[0] = (1/PCZ_AVGS[0]) * (geo_fact(zone=0,t=t) * (1-ALBEDO_SKYS[0])*(1-albedo_t(ALBEDO_AVGS[0],ALBEDO_ICE,abs(T[0]),ALBEDO_TEMP_DEPENDENT))*phi*SOLAR_CONST - EPSILON*TAU*SIGMA_B*T[0]**4) + thermal_exchange_rates[0]/(ZONE_SAREAS[0]*PCZ_AVGS[0])*(T[1]-T[0])

    for r in range(1,n-1):
        # calculate temperature at time t for each ODE
        ODEs[r] = (1/PCZ_AVGS[r]) * (geo_fact(zone=r,t=t) * (1-ALBEDO_SKYS[r])*(1-albedo_t(ALBEDO_AVGS[r],ALBEDO_ICE,abs(T[r]),ALBEDO_TEMP_DEPENDENT))*phi*SOLAR_CONST - EPSILON*TAU*SIGMA_B*T[r]**4) + 1/(ZONE_SAREAS[r]*PCZ_AVGS[r])*(-thermal_exchange_rates[r-1]*(T[r]-T[r-1]) + thermal_exchange_rates[r]*(T[r+1]-T[r]))

    ODEs[5] = (1/PCZ_AVGS[5]) * (geo_fact(zone=5,t=t) * (1-ALBEDO_SKYS[5])*(1-albedo_t(ALBEDO_AVGS[5],ALBEDO_ICE,abs(T[5]),ALBEDO_TEMP_DEPENDENT))*phi*SOLAR_CONST - EPSILON*TAU*SIGMA_B*T[5]**4) - thermal_exchange_rates[4]/(ZONE_SAREAS[5]*PCZ_AVGS[5])*(T[5]-T[4])
...
#     GAMMAS = [0.18321732, 0.23004537,0.28175096,0.28175096,0.23004537,0.18321732]

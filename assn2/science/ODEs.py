...
from .forcings import solarflux, phi_volcano 
...
def ODEs_T(t,T, VOLC=False):
    ...
    ODEs[0] = (1/PCZ_AVGS[0]) * (GAMMAS[0] * (1-ALBEDO_SKYS[0])*(1-ALBEDO_AVGS[0])*phi*SOLAR_CONST - TAU*SIGMA_B*T[0]**4) + thermal_exchange_rates[0]/(ZONE_SAREAS[0]*PCZ_AVGS[0])*(T[1]-T[0])

    for r in range(1,n-1):
        # calculate temperature at time t for each ODE
        ODEs[r] = (1/PCZ_AVGS[r]) * (GAMMAS[r] * (1-ALBEDO_SKYS[r])*(1-ALBEDO_AVGS[r])*phi*SOLAR_CONST - TAU*SIGMA_B*T[r]**4) + 1/(ZONE_SAREAS[r]*PCZ_AVGS[r])*(-thermal_exchange_rates[r-1]*(T[r]-T[r-1]) + thermal_exchange_rates[r]*(T[r+1]-T[r]))

    ODEs[5] = (1/PCZ_AVGS[5]) * (GAMMAS[5] * (1-ALBEDO_SKYS[5])*(1-ALBEDO_AVGS[5])*phi*SOLAR_CONST - TAU*SIGMA_B*T[5]**4) - thermal_exchange_rates[4]/(ZONE_SAREAS[5]*PCZ_AVGS[5])*(T[5]-T[4])

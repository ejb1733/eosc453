...
from .forcings import phi_volcano, albedo_t
from data.constants import *

okwtf = 1

# set year at which eruptions will occur
eruption_year = 25
...
def ODEs_T(t,T, VOLC=True, ALBEDO_TEMP_DEPENDENT=True):
    ...
    # set default for volcanism forcing (1 == no volcanism)
    phi = 1

    # ODEs for zone 1
    ODEs[0] = (1/PCZ_AVGS[0]) * (GAMMAS[0] * (1-ALBEDO_SKYS[0])*(1-albedo_t(ALBEDO_AVGS[0],ALBEDO_ICE,abs(T[0]),ALBEDO_TEMP_DEPENDENT))*SOLAR_CONST - EPSILON*TAU*SIGMA_B*T[0]**4) + thermal_exchange_rates[0]/(ZONE_SAREAS[0]*PCZ_AVGS[0])*(T[1]-T[0])

    # set volcano to act only in zone 3
    ...
    # ODEs for zone 6
    ODEs[5] = (1/PCZ_AVGS[5]) * (GAMMAS[5] * (1-ALBEDO_SKYS[5])*(1-albedo_t(ALBEDO_AVGS[5],ALBEDO_ICE,abs(T[5]),ALBEDO_TEMP_DEPENDENT))*SOLAR_CONST - EPSILON*TAU*SIGMA_B*T[5]**4) - thermal_exchange_rates[4]/(ZONE_SAREAS[5]*PCZ_AVGS[5])*(T[5]-T[4])
